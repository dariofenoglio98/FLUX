...
METHOD: in the first round, the server sends the global model to all clients for evaluation and descritor extraction.
Then, the server clusters the clients based on the extracted descriptors. The server aggregates the models of the clients
in each cluster and sends the aggregated model to the clients in the respective cluster for training. The server also sends
the aggregated clustered model to the clients in the respective cluster for evaluation. 
In the next round of training, the server continues to cluster the clients based on the extracted descriptors using the 
previous round's clustered model (not great-different models used for evaluation). 
"""
